...
*/
		...
		{
            //TODO: Then sort the par list by size relative to parlist filenames using <filename> in regex (Thus only returning the least value par)

            System.Text.RegularExpressions.Regex par2Pattern = new System.Text.RegularExpressions.Regex("(.+(\\.par2)(\\s|\"))", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
            //The .par2 could be followed by a space or "

            ArrayList pars = new ArrayList();

            //Find all par2 articles
            foreach (Article article in m_Articles)
            {
                if (par2Pattern.IsMatch(article.Subject.ToString()))
                {
                    pars.Add(article);
                }
            }
        ...
        public ArrayList GetSmallParList(ArrayList parList)
        {
            System.Text.RegularExpressions.Regex filenamePattern = new System.Text.RegularExpressions.Regex("(.+(\\.par2)(\\s|\"))", System.Text.RegularExpressions.RegexOptions.IgnoreCase);

            ArrayList smallPars = new ArrayList();
